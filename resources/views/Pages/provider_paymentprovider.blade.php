    ...
    <div class="card mt-3">
        ...
        <div class="container-fluid mt-2">
                ...
                <div>
                    <div class="mt-2">
                        ...
                        <button type="button" class="btn btn-success float-end mr-2 mt-2" data-toggle="modal"
                            data-target="#insertpaymentModal">เพิ่มช่องทางการชำระ</button>
                    </div>


                </div>
                <!-- Insert Modal -->
                <div wire:ignore.self class="modal fade" id="insertpaymentModal" tabindex="-1"
                    aria-labelledby="insertpaymentModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header card-header bg-success border-success">
                                <h5 class="modal-title" id="insertpaymentModalLabel">เพิ่มช่องทางการรับเงิน</h5>
            ...
            <table class="table table-bordered mt-3 dtr-inline" id="example1" aria-describedby="example1_info">
                        ...
                        <tr>
                            ...
                            data-target="#viewstatusModal">ดูรายละเอียด</button> --}}
                                <button type="button" class="btn btn-warning" data-toggle="modal"
                                    data-target="#editpaymentModal{{ $row->id }}"><i class="fas fa-edit"></i></button>
                ...
                <!-- edit Modal -->
                <div wire:ignore.self class="modal fade" id="editpaymentModal{{ $row->id }}" tabindex="-1"
                    aria-labelledby="editpaymentModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header card-header bg-warning border-warning">
                                <h5 class="modal-title text-white" id="editpaymentModalLabel">แก้รายละเอียดผู้ประกอบการ
                                </h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            <form action="{{ url('update-payment', $row->id) }}" enctype="multipart/form-data">
                                @csrf
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="mt-3">
                                        <label>เลขที่บัญชี</label>
                                        <input type="text" name="bank_account_no" class="form-control" maxlength="10"
                                            value="{{ $row->bank_account_no }}" required>
                                        @error('bank_account_no')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
